...
<head>
    ...
    <title>Clientes — Electrodomésticos</title>
    ...
    <style>
        ...
        .cli-table tr:hover td { background:var(--elec-light); }
        .avatar { width:36px;height:36px;border-radius:50%;background:var(--elec-light);color:var(--elec);font-weight:800;font-size:13px;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1.5px solid var(--elec); }
        ...
        .btn-sm:hover { background:var(--elec);color:#fff;border-color:var(--elec); }
        ...
        @media(max-width:600px) { .cli-table { display:none; } .filter-bar,.stats-bar,.elec-toolbar { padding:12px; } }
    </style>
</head>
<body>
...
<div class="dashboard-layout">
    ...
    <div class="main-content" style="flex:1;overflow-y:auto;padding:0;">
        ...
        <div class="elec-toolbar">
            <div>
                <h1><i class="fas fa-tv" style="color:var(--elec);margin-right:8px;"></i>Clientes</h1>
                <p id="subtitulo">Cargando…</p>
            </div>
            <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                <a href="servicios.php" class="btn btn-secondary" style="text-decoration:none;"><i class="fas fa-screwdriver-wrench"></i> Órdenes de Servicio</a>
                <button class="btn btn-primary" onclick="abrirModal()"><i class="fas fa-plus"></i> Nuevo Cliente</button>
            </div>
        </div>
        <div class="stats-bar">
            <div class="stat-pill"><div class="dot" style="background:var(--elec);"></div><span id="st-total">0</span> Clientes</div>
            <div class="stat-pill"><div class="dot" style="background:#ef4444;"></div><span id="st-activos">0</span> Con servicio activo</div>
            <div class="stat-pill"><div class="dot" style="background:#0FD186;"></div><span id="st-listos">0</span> Listos p/retirar</div>
        </div>
...
<div class="modal-overlay" id="modalCli">
    <div class="modal-box">
        <div class="modal-header">
            <h3 id="modalTitulo"><i class="fas fa-user-plus" style="color:var(--elec);margin-right:8px;"></i>Nuevo Cliente</h3>
...
<script>
const BASE = '<?= $base ?>';
const API  = BASE + '/api/electrodomesticos/clientes.php';
let todos = [], filtrados = [];

async function init() {
    const r = await fetch(API, {credentials:'include'});
    const j = await r.json();
    if (!j.success) { document.getElementById('cliContent').innerHTML = `<div style="text-align:center;padding:60px;color:var(--text-secondary);"><p>${j.message}</p></div>`; return; }
...
function render() {
    ...
    if (!filtrados.length) {
        cont.innerHTML = `<div style="text-align:center;padding:60px;color:var(--text-secondary);">
            ...
            <p style="font-size:16px;font-weight:600;margin-bottom:16px;">No hay clientes</p>
            <button class="btn btn-primary" onclick="abrirModal()"><i class="fas fa-plus"></i> Nuevo Cliente</button>
        </div>`;
        return;
    }
    cont.innerHTML = `<table class="cli-table"><thead><tr>
        <th>Cliente</th><th>DNI</th><th>Teléfono</th><th>Servicios</th><th>Activos</th><th>Listos</th><th></th>
    </tr></thead><tbody>
        ${filtrados.map(c => `<tr>
            <td><div style="display:flex;align-items:center;gap:10px;">
                <div class="avatar">${esc((c.apellido||c.nombre||'?')[0]).toUpperCase()}</div>
                <div><div style="font-weight:700;">${esc(c.apellido?c.apellido+', '+c.nombre:c.nombre)}</div>
                ${c.email?`<div style="font-size:11px;color:var(--text-secondary);">${esc(c.email)}</div>`:''}</div>
            </div></td>
            <td>${c.dni||'—'}</td>
            <td>${c.telefono?`<a href="tel:${esc(c.telefono)}" style="color:var(--elec);font-weight:600;">${esc(c.telefono)}</a>`:'—'}</td>
            <td><span class="badge badge-zero">${c.total_servicios||0}</span></td>
            <td>${(c.servicios_activos>0)?`<span class="badge badge-activo">${c.servicios_activos}</span>`:'—'}</td>
            ...
            <td><div style="display:flex;gap:6px;">
                ...
                <a href="servicios.php?cliente=${c.id}" class="btn-sm" style="text-decoration:none;" title="Ver servicios"><i class="fas fa-screwdriver-wrench"></i></a>
            </div></td>
        </tr>`).join('')}
    </tbody></table>`;
}
...
function abrirModal(c=null) {
    ...
    document.getElementById('modalTitulo').innerHTML = c
        ? `<i class="fas fa-edit" style="color:var(--elec);margin-right:8px;"></i>Editar Cliente`
        : `<i class="fas fa-user-plus" style="color:var(--elec);margin-right:8px;"></i>Nuevo Cliente`;
...
async function guardar() {
    ...
    const body = {
        ...
        direccion:    document.getElementById('cliDir').value.trim()||null,
